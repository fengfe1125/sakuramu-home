...
顶栏、区块、卡片、键值行、页脚。由 `scripts/build-shared.mjs` 注入两个页面的
...
| 名字 Rowan 从左往右手写出来，写到一半右栏和插画块浮上来；口号下的弧线、「写信给我」后面的箭头、插画块上的手写批注和箭头依次画出来 | 首页首屏 |
...
正常情况下 `shared/motion.js` 等字体到了再加 `.play` 开播（最多等 1 秒），
免得名字先用系统字体写一半、再突然换成手写体；
页面脚本跑完（`DOMContentLoaded`）1.5 秒后无论如何会加 `.play`，哪段脚本出错首屏也不会一直藏着。
兜底从 `DOMContentLoaded` 起算：字体样式表慢的时候后面的脚本要等它，从页面一开始算的话兜底会抢在字体前面。
