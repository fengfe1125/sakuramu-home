...
4. 身份验证：选中 **Cloudflare** 和 **One-time PIN** 两个登录方式。只选一个时「应用即时身份验证」会自动打开；两个都选时要把它关掉，登录页才会让你挑
...
- **Cloudflare**：用 Cloudflare 账号登录（账号密码；账号开了二次验证的话，还要再验证一步）。
- **Email**：输入邮箱，收一个一次性验证码。

两种都留着，是因为各有卡住的时候：验证码邮件可能收不到；换一台没登录过 Cloudflare 的设备，验证码又是最省事的办法。
多一个 Cloudflare 登录不会变得不安全 —— 能登上 Cloudflare 账号的人，本来就能改整个网站。
...
**策略里没有的邮箱，Access 一封邮件都不会发** —— 但页面照样显示「验证码已发送」，
Access 的登录日志里也什么都看不到（只有提交过验证码才会留记录）。
所以「收不到验证码」最常见的原因是填了一个不在策略里的邮箱，而不是邮件丢了。
2026-09-14 那次就是这样：填的是 Cloudflare 账号的邮箱，策略里只有另一个。

现在策略里放了两个邮箱：常用来收验证码的那个，和 Cloudflare 账号的邮箱（用 Cloudflare 登录时，报出来的就是它）。两个都能收验证码。

邮箱确认没填错的话，再去垃圾邮件里找 `noreply@notify.cloudflare.com` 发来的信。

### 放行名单有两处，要一致

**策略里的邮箱必须包含登录方式报出来的那个邮箱。** 不一致会出现「登录成功、随后被策略拒绝」，
界面上看不出是哪一步出的问题 —— 这是这套东西最不直观的失败模式。

`ACCESS_ALLOWED_EMAILS` 这道 Worker 侧的闸也要放同样的两个邮箱（逗号分隔），
...
等待网络属于 I/O 不计 CPU，真正花 CPU 的是字符串处理 —— 主页 HTML 约 78KB，
超过了这 64KB，但关键字「沐枫」就在开头的 `<title>` 里，读前 64KB 足够；
...
两个页面的外观来自同一份设计系统 `shared/base.css`：颜色、字体、网格、
顶栏、区块、卡片、键值行、按钮、页脚。由 `scripts/build-shared.mjs` 注入两个页面的
`base:start` / `base:end` 之间（同一个脚本也负责注入 `shared/beacon.js` 和
`shared/motion.js`）。改样式改那一个文件，两边同时生效。页面各自特有的样式写在注入块**之后** ——
同优先级时后写的赢，页面可以放心覆盖共用的默认值。

风格是线条简约：颜色取自 anthropic.com 的实测值（象牙白底、墨色字、陶土色点缀），
结构全靠 1px 发丝线，不用阴影和渐变。陶土色在浅底上对比度只有 3:1，
只给色块、线条和大字用，小字一律用 `--ink-3`。深色模式跟随系统。

手写体只做点缀：中文龙藏体（Long Cang），英文 Caveat，都走 Google Fonts。
用在首屏口号「Dare to think.」、插画块上的批注、区块标题旁的一句小话和页脚签名；
正文和标题不用手写。中文字体按字切片，页面只下载用到的那几个字所在的分片。
...
| 动效 | 在哪 |
|---|---|
| 站标的猫一笔画出，眼睛最后点上；之后每隔几秒眨一次眼，指向站标时头顶的鱼会晃 | 两个页面的顶栏 |
| 名字 Rowan 逐字浮起，右栏和插画块跟着浮现；口号下的弧线、插画块上的手写批注和箭头依次画出来 | 首页首屏 |
| 区块顶部的分隔线滚到时从左往右画出，旁边的手写小话、页脚的手写签名跟着从左往右「写」出来；顶栏底边一条陶土色细线跟着阅读进度走 | 两个页面 |
| 六个统计数字从 0 滚到实际值，热力图逐列出现，工具用量的线条长出来 | 首页「用量」 |
| 鼠标在插画块上移动能拨动四维立方体，卡片悬停轻轻浮起 | 首页 |

**动画绝不能是内容可见的前提。** `<head>` 里一段小脚本决定播不播：开了「减少动态效果」、
或页面在后台标签页里打开时，不加 `.motion`，一切直接是终态；
`.motion` 加上后 1.5 秒无论如何会加 `.play`，后面哪段脚本出错首屏也不会一直藏着。
数字滚动之前终值就已经写在页面上，后台标签页里 `requestAnimationFrame` 不跑，另有定时器保证终值落地。

揭示动画的 2.5 秒兜底只在 `IntersectionObserver` 的回调**一次都没来过**时才生效
（observe 时每个目标都会先报一次，在不在视口里都报）。如果只看「有没有块露出来」，
首屏没有揭示块时兜底会在 2.5 秒后把整页提前亮完，往下滚就什么动画都看不到了。
...
`npm run deploy` / `npm run deploy:about` 在上传之前会跑
`scripts/check-placeholders.mjs`，页面里还有 `data-todo` 就拒绝部署，并指出在哪一行。

之前关于页的「草稿待改」提示框就是跟着一起发到线上的 —— 写给作者看的东西，
不能靠记性拦住。
