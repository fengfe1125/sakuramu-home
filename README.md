...
`scripts/refresh-snapshot.mjs` 在发版前把线上载荷写进 `public/index.html`。
两条硬规矩：

- **拉取或校验失败不阻断发版** —— 保留现有快照、大声警告、退出码 0。
  统计服务偶尔抽风不该让主站发不出去。要在 CI 里当硬失败就加 `--strict`。
- **写入前做结构校验** —— 检查格式版本、数组长度与 `range.days` 是否一致、
  逐日求和与 `window_tokens` 是否相符、`dated + undated == tokens` 恒等式。
  宁可留着旧快照，也不把坏数据烧进页面。

端点和用户名可用环境变量覆盖：`TT_ENDPOINT` / `TT_HANDLE`。
